# Blipyy Mobile Support Migration Guide

This guide explains how to upgrade your existing Blipyy deployment to support mobile applications with both cloud and self-hosted options.
...
This script will:
1. [SEARCH] Detect your running Blipyy containers
...
```bash
# Create a backup
docker exec blipyy-db pg_dump -U trader -d blipyy > backup_$(date +%Y%m%d).sql
```

#### Step 2: Run Database Migrations
```bash
# Copy the migration script to your container
docker cp backend/src/utils/migrate.js blipyy-app:/app/backend/src/utils/

# Copy migration files
docker cp backend/migrations/ blipyy-app:/app/backend/

# Run migrations
docker exec blipyy-app node /app/backend/src/utils/migrate.js
```
...
```bash
# Restart your Blipyy containers
...
GET /.well-known/blipyy-config.json
```
...
The mobile app will automatically detect if it's connecting to `blipyy.io` (cloud) or a self-hosted instance based on the domain.
...
2. **Auto-Discovery**: App fetches `/.well-known/blipyy-config.json`
...
```bash
docker exec blipyy-db psql -U trader -d blipyy -c "\dt"
```
...
curl https://your-domain.com/.well-known/blipyy-config.json
```
...
3. Check logs: `docker logs blipyy-app`
...
2. **Restore database**:
   ```bash
   docker exec -i blipyy-db psql -U trader -d blipyy < your_backup.sql
   ```
